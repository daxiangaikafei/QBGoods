      ...
      reactSwipe = <ReactSwipe ref="swiper" className="carousel" swipeOptions={{continuous: false, callback: this.swiperCallback}}>
                    {
                      this.props.swipers.map(function(item,i){
                        return (<div key={i}><a data-event="point" data-event-stuffMoudId="钱宝自营" data-event-type="shop" data-event-id={item.id} data-event-locationId="21" data-event-source={item.source} href={item.linkUrl}><img src={item.imgUrl}/></a></div>)
                      })
                    }
                  </ReactSwipe>;
